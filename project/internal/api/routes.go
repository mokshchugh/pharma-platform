...
// registerRoutes mounts the full API route set shared by every server
// variant. Routes(), which additionally mounts the embedded SPA, and
// RoutesBackend(), which doesn't, both build on this single definition
// so the two route sets can never drift apart.
...
// Routes returns the full route set plus the embedded SPA mounted at "/*".
// Used by server variants that serve the frontend themselves.
...
// RoutesBackend returns the API route set with no SPA mount. Used when the
// frontend is served separately (e.g. the Vite dev server).
